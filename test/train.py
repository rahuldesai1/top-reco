...
def train(data_path, output_path, max_num_epochs, number_layers, batch_size, optim, learning_rate, dropout):
	print("[PROCESSING] Loading Data")
	final_df = pd.read_csv(data_path, delimiter=',')
	...
	final_dict["Test_Results"].append({"Signal Efficiency": test_acc[0], "Background Acceptance": test_acc[1]})
	
	save_df = save_df.append(final_dict, ignore_index=True)
	save_df.to_json("{0}/model_metrics.json".format(output_path))
	...
	with open("{0}/model_results.txt".format(output_path), "w+") as f:
		print("[INFO] Writing model metrics to model_results.txt")
		f.writelines([df_pred])
    
	save_path = saver.save(sess, "{0}/model".format(output_path))
