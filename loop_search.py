...
def train(sample_frac, number_layers, batch_size, optim, learning_rate, dropout, weight):
    print(sample_frac, number_layers, batch_size, optim, learning_rate, dropout, weight)
    optim = eval(optim)
    ...
    class_weights = tf.constant([0.1, weight])
    ...
    final_dict["HyperParameters"].append({"Layers":number_layers, "Batch Size": batch_size, "Learning Rate": learning_rate, "Optimizer": "AdamOptimizer", "Dropout": dropout, "Pos Weight": weight})
    ...
    for n in [11, 13]:
        for b in [512]:
            for l in [0.001, 0.0001]:
                for d in [0.5]:
                    for w in [0.8, 1.0, 1.4]:
                        hyperparameters.append((s, n, b, "tf.train.AdamOptimizer", l, d, w))

for sample_frac, number_layers, batch_size, optim, learning_rate, dropout, weight in hyperparameters:
    #try:
    train(sample_frac, number_layers, batch_size, optim, learning_rate, dropout, weight)
    #except Exception as e:
        #print(e)
        #print("Failed on input {0}".format((sample_frac, number_layers, batch_size, optim, learning_rate, dropout, weight)))
